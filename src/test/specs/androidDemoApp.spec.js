...
describe("API Demo Android APP tests", () => {
  ...
  it("should validate all menu items", async () => {
    const actualMenuItems = await MainMenuPage.allMenuItemsElements.map(
      async (menuItem) => await menuItem.getText()
    );
  ...
  it("should validate command two menu with app activity", async () => {
    ...
    await expect(AlertAppPage.commandTwoMsgElement).toHaveText(
      testdata.COMMAND_TWO_POPUP_MSG
    );
  });
  ...
  it("should validate screen top sendkeys", async () => {
    ...
    await expect(AutoCompletePage.countryInputElement).toHaveText(
      testdata.country
    );
  });
  ...
  it("should validate vertical scrolling", async () => {
    ...
    await expect(wallpaperPage.wallpaperTextElement).toHaveTextContaining(
      testdata.WALLPAPER_TEXT
    );
  });
